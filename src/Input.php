...
final class Input {
	...
	public function value($value, $type = null) {
		...
		$filtered = \filter_var($value, self::determineFilterMode($type), self::composeFilterFlags());
	...
	private static function fromSource($source, $elements, $key, $type = null) {
		...
		$filtered = \filter_input($source, $key, self::determineFilterMode($type), self::composeFilterFlags());
	 ...
	 */
	private static function composeFilterFlags() {
		$flags = 0;
